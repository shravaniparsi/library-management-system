                ...
                <v-card-text>
                  <v-form ref="form" lazy-validation>
                    ...
                    <router-link :to="'register'"
                      >Not a member?sign up</router-link
                    >
                  </v-form>
                </v-card-text>
              </v-card>
  ...
  methods: {
    validate() {
      if (this.$refs.form.validate()) {
        this.$http
          .post("http://localhost:3000/api/Students/login", this.u, {
    ...
    fetch(cb) {
      this.$http.get("http://localhost:3000/api/books").then(
        result => {
    ...
    fetchData(cb) {
      ...
      let url =
        "http://localhost:3000/api/transactions?filter[include]=Book&filter[where][user_Id]=" +
    ...
    fetchProfileData(cb) {
      ...
      this.$http
        .get("http://localhost:3000/api/Students/" + localStorage.userId, {
          headers: headers
        })
        .then(
          result => {
